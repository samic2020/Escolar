...
Public Class santander
    ...
    Private Function FatorVencimento(fator As String, vencimento As String) As String
        Dim retorno As String = "0000"
        If (vencimento.Length() < 8) Then
            retorno = "0000"
        Else
            retorno = CStr(DateDiff(DateInterval.Day, CDate(fator), CDate(vencimento)))
        End If
        Return retorno
    End Function

    Private Function CalcDig11N(cadeia As String) As String
        Dim total As Integer = 0
        Dim mult As Integer = 2
        For i = cadeia.Length To 1 Step -1
            total += Val(Strings.Mid(cadeia, i, 1)) * mult
            mult += 1
            If mult > 9 Then mult = 2
        Next
        Dim soma As Integer = total
    ...
    Private Function CalcDig11NBarrasSantander(cadeia As String) As String
        Dim total As Integer = 0
        Dim mult As Integer = 2
        For i = cadeia.Length To 1 Step -1
            ' total += Val(cadeia.Substring(startIndex:=cadeia.Length() - i, length:=cadeia.Length() + 1 - i)) * mult
            total += Val(Strings.Mid(cadeia, i, 1)) * mult
            mult += 1
            If mult > 9 Then mult = 2
        Next
        Dim soma As Integer = total * 10
        Dim resto As Integer = (soma Mod 11)
    ...
    Private Function Modulo10(cadeia As String) As String
        Dim total As Integer = 0
        Dim mult As Integer = 2
        For i = cadeia.Length To 1 Step -1
            Dim soma As Integer = Val(Strings.Mid(cadeia, i, 1)) * mult
            If soma > 9 Then
                Dim ssoma As String = soma.ToString
                soma = Val(Strings.Mid(ssoma, 1, 1)) + Val(Strings.Mid(ssoma, 2, 1))
            End If
            total += soma
            If mult = 2 Then
                mult = 1
            ...
            End If
        Next
        Dim resto As Integer = total Mod 10
        If resto > 0 Then
            resto = 10 - resto
        End If
    ...
    Private Function CodBar(vencimento As String, valor As String, nossonumero As String) As String
        Dim bco As New bancos
        bco.LerBanco("033")
        Dim part1 As String = bco.getBanco & bco.getMoeda
        Dim part2 As String = FatorVencimento("10/07/1997", vencimento) + bco.Valor4Boleta(valor) + "9" + Strings.Left(bco.getIdentificacao(), 7) + nossonumero + "0" + bco.getCarteira
        Dim dv As String = CalcDig11NBarrasSantander(part1 & part2)
        Return part1 & dv & part2
    End Function

    Private Function LinhaDigitavel(nossonumero As String, codbardv As String, vencimento As String, valortitulo As String) As String
        Dim campo1, campo2, campo3, campo4, campo5 As String
        Dim bco As New bancos
        bco.LerBanco("033")

        campo1 = bco.getBanco() & bco.getMoeda() & "9" & Strings.Left(bco.getIdentificacao(), 4)
        campo1 &= "." & Modulo10(campo1)

        campo2 = Strings.Right(bco.getIdentificacao(), 3) & Strings.Left(Strings.Right(nossonumero, 13), 7)
        campo2 &= "." & Modulo10(campo2)

        campo3 = Strings.Right(Strings.Right(nossonumero, 13), 6) & "0" & bco.getCarteira()
        campo3 &= "." & Modulo10(campo3)

        Dim dvcb As String = bco.getBanco() & bco.getMoeda() & FatorVencimento("10/07/1997", vencimento) &
                       valortitulo & "9" & Strings.Left(bco.getIdentificacao(), 7) &
                       Strings.Right(nossonumero, 13) & "0" & bco.getCarteira()
        campo4 = CalcDig11NBarrasSantander(dvcb)

        campo5 = FatorVencimento("10/07/1997", vencimento) & valortitulo

        Return campo1 & "  " & campo2 & "  " & campo3 & "  " & campo4 & "  " & campo5
    End Function

    Private Function NossoNumero(value As String) As String
        Dim tam As Integer = 13 '// Total de digitos do nossonumero santander
        Dim valor1 As String = Strings.Right(StrDup(tam - 1, "0") & Str(Val(value.Replace(" ", ""))).Trim, tam - 1)
        Dim valor2 As String = CalcDig11N(valor1)
        ...
        Using writer As New StreamWriter(fileRemessa, True)
            ...
            For Each item As DataGridViewRow In Boletas.SelectedRows
                ...
                writer.WriteLine(
                    ...
                    CalcDig11N(bco.getAgencia()).PadRight(1) &
                    bco.getCtaDv().PadLeft(1, "0") &
                    bco.getConta().PadLeft(9, "0") &
                    bco.getCtaDv().PadLeft(1, "0") &
                    bco.getConta().PadLeft(9, "0") &
                    Space(2) &
                    StrZero(NossoNumero(item.Cells(2).Value.ToString.Trim).Trim, 13) &
                    "5" &
                    "1" &
                    "2" &
                    Space(1) &
    ...
    Private Function CalBarras(cons_agencia As String, cons_conta As String, var_nossonumero As String, var_valordocumento As String,
        ...
        Dim dvnossonumero As String = CalcDig11N(var_nossonumero)
        ...
        Dim var_fatorvencimento As String = FatorVencimento("10/07/1997", "" & var_datavencimento & "")
        ...
        Dim var_linhadigitavel As String = LinhaDigitavel("" & Strings.Right(var_nossonumero & dvnossonumero, 13) & "", "0", "" & var_datavencimento & "", "" & var_valor & "")
